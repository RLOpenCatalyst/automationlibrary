include_recipe 'docker_rl::dep_check'
...
if node['docker']['exec_driver'] == 'lxc'
  include_recipe 'docker_rl::cgroups'
  include_recipe 'docker_rl::lxc'
end
...
unless node['docker']['install_type'] == 'package'
  if node['platform'] == 'ubuntu' && Chef::VersionConstraint.new('< 13.10').include?(node['platform_version'])
    include_recipe "docker_rl::#{node['docker']['storage_driver']}" if node['docker']['storage_driver']
  end
...
include_recipe "docker_rl::#{node['docker']['install_type']}"
include_recipe 'docker_rl::group' unless node['docker']['group_members'].empty?
include_recipe "docker_rl::#{node['docker']['init_type']}"
