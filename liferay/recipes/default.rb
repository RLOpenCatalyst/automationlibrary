...
execute "change permissions" do
        command "chown -R liferay:liferay /opt/tomcat7/logs"
        action :run
ignore_failure true
end

# Create default catalina.pid file to prevent restart error for 1st run of coookbook.
file "/var/run/catalina.pid" do
  owner 'root'
  group 'root'
  mode '0755'
  action :create
  not_if { ::File.exist?("/var/run/catalina.pid") }
end
...
bash "Load EXT Environment" do
  ...
  EOH
	...
	if platform_family?("debian")
		...
		execute "Stop Liferay" do
			command "cd #{node['liferay']['install_directory']}/liferay/tomcat/bin
					 ./shutdown.sh"
			action :run
		end  
		
		ruby_block "wait to start life ray" do
			block do
				sleep 10
			end
		end
